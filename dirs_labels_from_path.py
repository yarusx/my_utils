...
for file in filename:
    ...
    if str(os.path.splitext(file)[0])[31].isupper() is True:
        ...
        if not str(temp_dirname) in cats_names: cats_names.append(str(temp_dirname))
    ...
    if str(os.path.splitext(file)[0])[31].isupper() is False:
        ...
        if not str(temp_dirname) in dogs_names: dogs_names.append(str(temp_dirname))

file_d = open(sub_dir_dog+"dogs_names.txt", "w")
file_d.write(str(dogs_names))
file_d.close()

file_d = open(sub_dir_cat+"cats_names.txt", "w")
file_d.write(str(cats_names))
file_d.close()
